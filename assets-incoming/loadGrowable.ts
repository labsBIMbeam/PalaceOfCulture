...
/**
 * Growable assets for "600 Billion".
 * Art is static (the .glb), state is data (the .growth.json). A single `progress`
 * value (0..1) drives per-part growth at runtime — nothing is baked into the model.
 */
...
/**
 * Load a growable asset and return a handle whose `setGrowth(progress)` scales
 * each part in. The base scale is read from the glTF, so models stay authoring-accurate.
 */
