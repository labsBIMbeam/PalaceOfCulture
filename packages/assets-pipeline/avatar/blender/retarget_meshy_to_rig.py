...
NEW = arg("--new"); IDLE = arg("--idle"); WALK = arg("--walk"); RUN = arg("--run")
OUT = arg("--out"); CID = arg("--id")

def log(*a): print("[retarget]", *a)
def imp(path):
    before = set(bpy.data.objects); bpy.ops.import_scene.gltf(filepath=path)
    return [o for o in bpy.data.objects if o not in before]
def of_type(objs, t): return [o for o in objs if o.type == t]
def world_bbox(o):
...
D = max(donor_meshes, key=lambda m: len(m.vertex_groups))   # the real body (most vertex groups)
idle = arm.animation_data.action; idle.name = "idle"
arm.data.pose_position = 'REST'; bpy.context.view_layer.update()
dmn, dmx = world_bbox(D); dh = dmx.z - dmn.z
dcx = (dmn.x + dmx.x) / 2; dcy = (dmn.y + dmx.y) / 2

# --- new mesh: import, join parts, align to donor body (scale to height, centre XY, feet to floor)
new = imp(NEW); meshes = of_type(new, "MESH")
if len(meshes) > 1:
...
N = meshes[0]; N.animation_data_clear()
nmn, nmx = world_bbox(N); s = dh / (nmx.z - nmn.z) if (nmx.z - nmn.z) else 1.0
N.scale = [v * s for v in N.scale]; bpy.context.view_layer.update(); nmn, nmx = world_bbox(N)
...
bpy.ops.object.select_all(action='DESELECT')
N.select_set(True); D.select_set(True); bpy.context.view_layer.objects.active = D
...
bpy.ops.object.select_all(action='DESELECT')
N.select_set(True); arm.select_set(True); bpy.context.view_layer.objects.active = arm
...
def grab_action(path, name):
    ...
    objs = imp(path)
    acts = [o.animation_data.action for o in of_type(objs, "ARMATURE")
            if o.animation_data and o.animation_data.action]
    if acts: acts[0].name = name
    bpy.ops.object.select_all(action='DESELECT')
    for o in objs: o.select_set(True)
    bpy.context.view_layer.objects.active = objs[0]; bpy.ops.object.delete()
...
for o in stray: o.select_set(True)
if stray:
    bpy.context.view_layer.objects.active = stray[0]; bpy.ops.object.delete()
