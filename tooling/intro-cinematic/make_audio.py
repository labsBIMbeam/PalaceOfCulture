...
PENTA = [523.25, 622.25, 698.46, 783.99, 932.33, 1046.50, 1244.51]  # C5 Eb5 F5 G5 Bb5 C6 Eb6

LEFT = [0.0] * N
RIGHT = [0.0] * N


def add(t0: float, dur: float, freq, env, amp: float, pan: float = 0.0,
        harmonics: tuple[tuple[float, float], ...] = ((1.0, 1.0),)) -> None:
...
def chirp(t0: float, synthetic: bool) -> None:
    ...
    for base, dur in notes:
        if synthetic:
            ...
            amp = 0.14
        add(t, dur, freq, gauss_env(dur, 0.015), amp, 0.06 if synthetic else -0.04, harm)
        t += dur + gap
...
def shimmer() -> None:
    ...
    while t < 14.5:
        ...
        add(t, dur, const(f), gauss_env(dur, 0.018), RNG.uniform(0.045, 0.085),
            RNG.uniform(-0.7, 0.7))
...
def whum_and_ping() -> None:
    ...
    add(14.95, 0.55, const(880.0), exp_env(0.16, 0.003), 0.15, 0.05,
        ((1.0, 1.0), (2.0, 0.4)))
...
def zap_sparks() -> None:
    ...
    for i, f in enumerate(PENTA):
        ...
        add(t0, 0.34, const(f), exp_env(0.11, 0.002), 0.16, pan,
